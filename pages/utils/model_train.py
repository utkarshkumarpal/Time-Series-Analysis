...
def get_rolling_mean(closing_price):
    rolling_price=closing_price.rolling(window=7).mean().dropna()
...
def evaluate_model(original_price,differencing_order):
    test_data,train_data=original_price[-30:],original_price[:-30]
    predictions=fit_model(train_data,differencing_order)
    rmse=np.sqrt(root_mean_squared_error(test_data,predictions))
    return round(rmse,2)
